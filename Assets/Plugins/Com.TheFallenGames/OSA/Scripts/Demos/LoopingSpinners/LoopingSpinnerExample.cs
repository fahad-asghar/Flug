...
namespace Com.TheFallenGames.OSA.Demos.LoopingSpinners
{
	...
	public class LoopingSpinnerExample : OSA<MyParams, MyItemViewsHolder>
	{
		...
		protected override void Update()
        {
			...
			{
				int index = int.Parse(Content.GetChild(i).GetChild(0).GetComponent<Text>().text);
				DisableAll(Content.GetChild(i));
				Content.GetChild(i).GetChild(index + 1).gameObject.SetActive(true);
			}
		...
		private void DisableAll(Transform trans)
		{
